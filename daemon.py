...
#  2. POST /auth/server with response and gsid,
#     response is session key and limits
#  New Announcement:
...
class VoxRESTService(WZService, JinjaMixin):
    def __init__(self, config: Dict[str, Any]):
        ...
        self.url_map = Map(
            [
                ...
                Rule(
                    "/announcement/listen/<id>", endpoint="announcement_listen"
                ),
            ]
        )
    ...
    MSG_403 = json.dumps(
        {"error": True, "source": "server", "message": "Access Denied"}
    )
    ...
    MSG_400 = json.dumps(
        {"error": True, "source": "server", "message": "Bad Request"}
    )
    ...
    def handle_auth_server(self, request: Request) -> Response:
        ...
        if (
            request.method == "POST"
            and "response" in request.form
            and "gsid" in request.form
            and "challenge" in request.form
        ):
            ...
            if gsid not in self.config["gameservers"]:
                log.info(
                    "Failed GS %r login from address %s",
                    gsid,
                    request.remote_addr,
                )
            ...
            expected = hashlib.md5(
                (challenge + gss.secret_key).encode("utf-8")
            ).hexdigest()
            if response != expected:
                log.info(
                    "Failed GS %r login from address %s (%s != %s)",
                    gsid,
                    request.remote_addr,
                    response,
                    expected,
            ...
            log.info(
                "[%s] Successfully logged in from address %s",
                gss.id,
                request.remote_addr,
            )
            return self.jsonify(
    ...
    def handle_announcement_new(self, request: Request) -> Response:
        ...
        for k in ["auth", "phrase", "voice", "ckey", "gsid"]:
            if k not in request.form:
                log.error(
                    "%s missing from request.form (%r)", k, dict(request.form)
                )
                return self.make_400()
        ...
        if srvid not in self.gameservers:
            log.error(
                f"FAILED AUTH: {request.remote_addr} tried to request "
                f"an announcement as GSS {srvid}, which doesn't exist."
            )
            return self.make_403()
        ...
        if (
            authkey
            != hashlib.md5(
                (gss.session_key + phrase).encode("utf-8")
            ).hexdigest()
        ):
            log.error(
                "FAILED AUTH: %r via %r requested phrase %r with voice %r",
                ckey,
                request.remote_addr,
                phrase,
                voice,
        ...
        log.info(
            "[%s]: %s requested phrase %r with voice %r",
            srvid,
            ckey,
            phrase,
            voice,
...
def main():
    ...
    argp.add_argument(
        ...
        "-c",
        type=str,
        default=str(Path.cwd() / "daemon.yml"),
        help="Path of daemon.yml",
    )
    argp.add_argument(
        "--level",
        "-l",
    ...
    if not os.path.isfile("daemon.yml"):
        print(
            "Run `python tools/otftool.py init` first, then edit daemon.yml."
        )
        return
    ...
    run_simple(
        ...
        cfg["http"]["port"],
        app,
        use_debugger=cfg["http"].get("debug", False),
        use_reloader=False,
    )
