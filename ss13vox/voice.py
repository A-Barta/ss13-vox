...
# ("Installing the enhanced Nitech HTS voices" section)
# 'nitech_us_bdl_arctic_hts'
# 'nitech_us_jmk_arctic_hts'
# 'nitech_us_awb_arctic_hts'
# 'nitech_us_slt_arctic_hts'  # less bored US female
# 'nitech_us_clb_arctic_hts' # DEFAULT, bored US female
# (occasionally comes up with british pronunciations?!)
...
class Voice(object):
    ...
    ID: str = ""
    # Used to determine if voice goes in vox_fem or vox_mas,
    # also in idiot checks.
    ...
    def genSoxArgs(self, args) -> List[str]:
        ...
        if self.chorus:
            sox_args += [
                # Chorus adds harmonics, which makes it sound like multiple
...
class USRMSMale(Voice):
    """
    RMS US Male: Sounds a bit like DECTalk (Stephen Hawking).
    Uses US pronunciations.
    """
    ...
    def genSoxArgs(self, args) -> List[str]:
        sox_args = [
            ...
            # problems like noise spikes and dead spots.
            # 'synth', 'sine', 'amod', '55'
        ]
...
class ScotAWBMale(Voice):
    """
    AWB Scottish Male: Sounds a bit like DECTalk (Stephen Hawking).
    Uses US pronunciations.
    """
    ...
    def genSoxArgs(self, args) -> List[str]:
        sox_args = [
            ...
            # This was supposed to make it sound like the HL VOX
            # by increasing the gravelly quality by "gating" the incoming
            # stream, but it interferes with the voice's synth wave and
            # causes problems like noise spikes and dead spots.
            # 'synth', 'sine', 'amod', '55'
        ]
        return sox_args + super().genSoxArgs(args)
...
class USSLTFemale(Voice):
    """
    SLT US Female: More midwestern voice, talks faster,
    but buggy and occasionally drops into British.
...
class USCLBFemale(Voice):
    """
    CLB US Female: The /vg/station original vox_fem voice.
    Enunciates clearly, practically no bugs.
    """
