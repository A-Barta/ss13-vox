...
try:
    ...
    print(
        "pybuildtools not detected! Please run `python3 "
        "-m poetry install && sudo poetry run python setup.py`."
    )
    sys.exit(1)
...
REQUIRED_PACKAGES: Set[str] = {
    ...
    "vorbis-tools",  # oggenc
    "ffmpeg",
    # It's 2022. I would like to use Python 3.10
    # but it is still not widely adopted yet.
    "python3.9",
}

# These should still work, for now.
ARCHIVES_URL = "http://hts.sp.nitech.ac.jp/archives/"
NITECH_VOICES: Set[str] = {
    f"{ARCHIVES_URL}2.1/festvox_nitech_us_awb_arctic_hts-2.1.tar.bz2",
    f"{ARCHIVES_URL}2.1/festvox_nitech_us_bdl_arctic_hts-2.1.tar.bz2",
    f"{ARCHIVES_URL}2.1/festvox_nitech_us_clb_arctic_hts-2.1.tar.bz2",
    f"{ARCHIVES_URL}2.1/festvox_nitech_us_rms_arctic_hts-2.1.tar.bz2",
    f"{ARCHIVES_URL}2.1/festvox_nitech_us_slt_arctic_hts-2.1.tar.bz2",
    # 'http://dl.dropbox.com/u/1845335/release/
    # cmu_us_slt_arctic_hts.tar.gz', # Better but broken
    f"{ARCHIVES_URL}2.1/festvox_nitech_us_jmk_arctic_hts-2.1.tar.bz2",
    f"{ARCHIVES_URL}1.1.1/cmu_us_kal_com_hts.tar.gz",
    f"{ARCHIVES_URL}1.1.1/cstr_us_ked_timit_hts.tar.gz",
}
...
def InstallHTS():
    with log.info("Installing Nitech HTS voices:"):
        ...
        with os_utils.Chdir("hts_tmp"):
            ...
            os_utils.copytree(
                "lib/voices/us/", "/usr/share/festival/voices/us/"
            )
            os_utils.copytree(
                "lib/voices/us/", "/usr/share/festival/voices/us/"
            )
            os_utils.single_copy(
                "lib/hts.scm", "/usr/share/festival/hts.scm", as_file=True
            )
...
def FixHTS():
    with log.info("Fixing HTS..."):
        ...
        # -(require 'hts)
        # +(require 'hts21compat)
        fixes += [[re.compile(r"\(require \'hts\)"), "(require 'hts21compat)"]]
        # -(require_module 'hts_engine)
        # +(require_module 'hts21_engine)
        fixes += [
            [
                re.compile(r"\(require_module \'hts_engine\)"),
                "(require_module 'hts21_engine)",
            ]
        ]
        ...
        fixes += [
            [
                re.compile(r"\(Parameter.set \'Synth_Method \'HTS\)"),
                "(Parameter.set 'Synth_Method 'HTS21)",
            ]
        ]

        files_changed = 0
        for root, _, files in os.walk("/usr/share/festival/voices/us"):
            for filename in files:
                ...
                if filename.endswith(".scm"):
                    ...
                    if changes > 0:
                        ...
                        log.info(
                            " Fixed {} ({} changes)".format(filename, changes)
                        )
                        files_changed += 1
...
VALID_RELEASES: FrozenSet[str] = frozenset({"focal", "jammy"})
...
def main():
    print(
        "This script will set up your Ubuntu system for building VOX phrases."
    )
